...
func postInitialize(c echo.Context) error {
	...
	for rows.Next() {
		...
		_, err = db.Exec("UPDATE isu_condition SET `condition_level` = ? where `id` = ?", warnCount, condition.ID)
		if err != nil {
			c.Logger().Errorf("db error: %v", err)
			return c.NoContent(http.StatusInternalServerError)
		}
...
func genTrend() error {
	...
	for _, character := range characterList {
		...
		for _, isu := range isuList {
			conditions := []IsuCondition{}
			err = db.Select(&conditions,
				"SELECT * FROM `isu_condition` FORCE INDEX(jia_isu_uuid_timestamp) WHERE `jia_isu_uuid` = ? ORDER BY timestamp DESC LIMIT 1",
				isu.JIAIsuUUID,
			)
			if err != nil {
				log.Printf("db error: %v", err)
...
func postIsuCondition(c echo.Context) error {
	...
	for _, req := range splitPostIsuConditionRequest(req, 50) { // 50 は適当 随時変更する @@@ TODO FIXME
